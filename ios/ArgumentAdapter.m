...
RCRTCIWMediaType toMediaType(int type) { return (RCRTCIWMediaType)type; }

RCRTCIWCamera toCamera(int camera) { return (RCRTCIWCamera)(camera - 1); }
...
RCRTCIWRoomSetup *toRoomSetup(NSDictionary *arguments) {
  ...
  setup.joinType = toMediaType(type);
...
RCRTCIWCustomLayout *toLiveMixCustomLayout(NSDictionary *arguments) {
  ...
  RCRTCIWCustomLayout *layout =
      [[RCRTCIWCustomLayout alloc] initWithStreamType:type userId:uid tag:tag];
  layout.x = x;
  layout.y = y;
  layout.width = width;
...
NSArray<RCRTCIWCustomLayout *> *
toLiveMixCustomLayouts(NSArray<NSDictionary *> *arguments) {
...
RCRTCIWRole toRole(NSUInteger role) {
  static NSArray<NSNumber *> *RCRTCIWRole_array = nil;
  if (!RCRTCIWRole_array) {
    RCRTCIWRole_array = @[
      @(RCRTCIWRoleMeetingMember), @(RCRTCIWRoleLiveBroadcaster),
      @(RCRTCIWRoleLiveAudience)
    ];
  }
  if (role > [RCRTCIWRole_array count] - 1)
    role = [RCRTCIWRole_array count] - 1;

  return (RCRTCIWRole)[RCRTCIWRole_array[role] integerValue];
}
...
NSUInteger RoleToNum(RCRTCIWRole role) {
  ...
  return [RCRTCIWRole_array indexOfObject:@(role)];
}

RCRTCIWVideoFps toVideoFps(NSUInteger fps) {
  static NSArray<NSNumber *> *RCRTCIWVideoFps_array = nil;
  if (!RCRTCIWVideoFps_array) {
    RCRTCIWVideoFps_array = @[
      @(RCRTCIWVideoFps10), @(RCRTCIWVideoFps15), @(RCRTCIWVideoFps24),
      @(RCRTCIWVideoFps30)
    ];
  }
  if (fps > [RCRTCIWVideoFps_array count] - 1)
    fps = [RCRTCIWVideoFps_array count] - 1;

  return (RCRTCIWVideoFps)[RCRTCIWVideoFps_array[fps] integerValue];
}
...
NSUInteger VideoFpsToNum(RCRTCIWVideoFps fps) {
  ...
  return [RCRTCIWVideoFps_array indexOfObject:@(fps)];
}
